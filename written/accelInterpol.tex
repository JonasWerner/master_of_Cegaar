...
Because a program trace is rather abstract, the question arises whether the program will ever take the sequence of statements defined by a trace.
...
$\tau_0$ from before is infeasible, because for $\texttt{assume}$ statement $\texttt{!x < 6}$, the interpretation function $\mathcal{I}(\texttt{!x < 6})(x = 2)$ evaluates to false and is, with that, not part of the successor relation.
...
Assume we are given the following program trace $\tau_1$ from the example program $P_0$:
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[%
		...
		\node[state] (4) [right of=3] {$\ell_2$};
		\node[state] (5) [right of=4] {$\ell_3$};
		\node[state] (6) [below of=1] {$\ell_2$};
		\node[state] (7) [right of=6] {$\ell_3$};
		\node[state] (8) [right of=7] {$\ell_2$};
		\node[state] (9) [right of=8] {$\ell_4$};
		\node[state] (10) [right of=9] {$\ell_6$};
		
		
		\path (1) edge node {\texttt{x = 0}} (2);
		\path (2) edge node {\texttt{x < 6}} (3);
		\path (3) edge node {\texttt{x = x + 2}} (4);
		\path (4) edge node {\texttt{x < 6}} (5);
		\path (5) edge node {\texttt{x = x + 2}} (6);
		\path (6) edge node[below] {\texttt{x < 6}} (7);
		\path (7) edge node[below] {\texttt{x = x + 2}} (8);
		\path (8) edge node[below] {\texttt{!x < 6}} (9);
		\path (9) edge node[below] {\texttt{x != 6}} (10);
		;
	\end{tikzpicture}
	\captionof{figure}{Program Trace $\tau_1$.}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[%
		...
		\path (4) edge node {\texttt{x < 6}} (5);
		\path (5) edge node {\texttt{x = x + 2}} (6);
		\path (6) edge node[below right] {\texttt{x < 6}} (7);
		\path (7) edge node[below] {\texttt{x = x + 2}} (8);
		;
	\end{tikzpicture}
