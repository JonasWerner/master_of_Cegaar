...
\begin{figure}[H]
	\begin{center}
		\begin{tabular}{ccccccccccc}
			\loc{1} & \st{x:=0} & \loc{2} & \st{y:=1} & \loc{3} & \accel{1} & $\ell_{3}'$ & \st{x>50} & \loc{6} & \st{y!=103} & \loc{7} \\
		\end{tabular}
	\end{center}
...
Firstly, compute post($I_{\psi^*_L}$, $\psi^*_L$) as the loop entry interpolant $I_{\ell_L}$. 
...
\ts{The simplification of the interpolant at $\ell_6$ is wrong.}
...
To illustrate branching loops, assume we are given program $P_2 = (Loc_2, \delta_2, \ell_{1}, \ell_{9})$ depicted in Figure~\ref{fig:ex:p2}.
It is evident that $\ell_2$ is a loop head, it is, however, not possible to apply the technique described in the previous chapter as it is not possible to construct a single minimal loop trace. 
...
\begin{mydef}
	Given a program $P = (\Loc, \delta, \ell_\init, \ell_\err)$ and an error trace $\tau: s_0s_1\ldots\ s_n$ with loop trace $\tau_L: s_i, s_{i+1}, \ldots, s_j$. The loop is called branching, if there is a program location $\ell_B$ where there is a statement $s_k \in \tau_L\ \text{with}\ (\ell_B, s_k, \ell_l) \in \delta$ and there is a program statement $s_l \in \tau_L\  \text{with}\ (\ell_B, s_l, \ell_m) \in \delta$.
\end{mydef}
...
\begin{figure}[H]
	\begin{center}
		\begin{tabular}{ccccccccccc}
			\loc{1} & \st{x:=0}  & \loc{2} & \st{x<=10}  & \loc{3} & \st{x-4<=1} & \loc{5} & \st{x:=x+2} & \loc{6} & \st{x:=x+2} & \\
...
Because \begin{itemize}
	\item ($\ell_3$, \st{x-4<=1}, $\ell_5$) $\in \delta_2$ and \st{x-4<=1} $\in \tau_L$
	\item ($\ell_3$, \st{x-4>1}, $\ell_4$) $\in \delta_2$ and \st{x-4>1} $\in \tau_L$
\end{itemize}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[%
		...
		\node[state] (5) [left of=4, xshift=-15pt] {$\ell_5$};
