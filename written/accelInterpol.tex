...
\begin{figure}[H]
	...
	\begin{tikzpicture}[%
		...
		(8) edge[bend right] node[right=0.25cm] {\texttt{y := x + 5}} (7)
...
\begin{align*}
 	\psi_{L_1}&:	\texttt{x <= 100; x := x + 2; y := x + 5} \ \\
 	&= \{(\sigma, \sigma')\ |\ \sigma[x] \leq 100 \land \sigma'[x] = \sigma[x] + 2 \land \sigma'[y] = \sigma'[x] + 5 \}
\end{align*}
...
For our example loop relation  	$\psi_{L_1}$ we get the reflexive transitive closure:
\begin{align*}
	\psi^*_{L_1}:& \{(\sigma, \sigma') \  |\ (\sigma[x] = \sigma'[x]\ \land\ \sigma[y] = \sigma'[y])\ \\ &\lor\ (\sigma[x] \leq 100\ \land\ \sigma'[x] \leq 102\ \land\ \sigma'[x] = \sigma[x] + 2\ \land\ \sigma'[y] = \sigma[x] + 5) \}
\end{align*}

\jw{Todo: maybe proof, that every trace is in}
...
Reconsider program $P_1$ from the previous chapter. To prove its safety trace abstraction finds error trace $\tau_1$, as before, we detect a loop with loop head $\ell_3$ and minimal loop trace $\tau_{L_1}$ from which we construct the loop relation $\psi_{L_1}$ and consequently the loop acceleration $\psi^*_{L_1}$. \par
...
\path (4) edge node[] {\texttt{!x <= 200}} (5); \\
\path (5) edge node {\texttt{y != 207}} (6); \\
;
\end{tikzpicture}
	\captionof{figure}{$\bar{\tau_1}$}
\end{figure}
