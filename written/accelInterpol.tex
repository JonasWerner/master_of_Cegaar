...
\begin{figure}[H]
	\begin{tikzpicture}[%
		...
		\node[state, label=above:{$true$}](1){$\ell_1$};
		
		\node[state, label=above:{$x = 0$}] (2) [right of=1] {$\ell_2$};
		
		\node[state, label=above:{$y = 1 \land x = 0$}] (3) [right of=2] {$\ell_3$};
		

		
		\node[state, label={[align=left]above:$x = 0\ \lor$ \\ $(y \leq 103\ \land$ \\ $(102 < y\ \lor\ $ \\ $2x + 1 \leq y))$}] (4) [right of=3] {$\ell_3'$};
		
		\node[state, label=above:{$y = 103$}] (5) [right of=4, xshift=0.5cm] {$\ell_6$};
		
		\node[state, label=above:{$false$}] (6) [right of=5, xshift=0.5cm] {$\ell_7$};
