...
;
\end{tikzpicture}
	\captionof{figure}{Meta trace $\bar{\tau_1}$ Generated from $\tau_1$ and $\psi^*_{L_1}$.}
\end{figure}
...
\begin{figure}[H]
	\begin{tikzpicture}[%
		...
		\node[state, label={[align=left]above:{$y = 1\ \land$ \\$x = 0$}}] (3) [right of=2] {$\ell_3$};
...
\end{equation*}
We cannot, however, use $I_{\bar{\tau_1}}$ to disprove $\tau_1$ as we need an interpolant for each location in the trace. To remedy this, we derive an inductive interpolant sequence $I_{\tau_1}$ by using the post operator. \\ \\
...
With loop invariant: $y = 2\cdot c + 1\ \land\ x \leq 51$
