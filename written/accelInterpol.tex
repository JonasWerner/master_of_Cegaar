...
\begin{align*}
	I&: \tau_{L_1}:\ \texttt{x <= 10; !x - 4 > 1; x := x + 2;\ x := x + 2} \\
	II&: \tau_{L_2}:\ \texttt{x <= 10; x - 4 > 1; x := x + 1; x := x + 2}
\end{align*}
With loop relations:
\begin{align*}
	\psi_{L_{I}} &= \{(\sigma, \sigma')\ |\ \sigma[x] \leq 5 \land  \sigma'[x] = \sigma[x] + 4 \} \\
	\psi_{L_{II}}&= \{(\sigma, \sigma')\ |\ \sigma[x] \leq 10 \land \sigma[x] > 5 \land  \sigma'[x] = \sigma[x] + 3 \}
\end{align*}

Using these two relations we can model an iteration of the loop as $\tau_{L_1} \lor \tau_{L_2}$.  \\
\jw{TODO: use these}
...
\begin{figure}[H]
	\begin{tikzpicture}[%
		...
		;
	\end{tikzpicture}
	\captionof{figure}{Meta Trace $\bar{\tau_2}$ Generated from $\tau_2$ Using a Loop with Branching.}
\end{figure}
