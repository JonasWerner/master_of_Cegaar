...
\begin{mydef}
	Given an error trace $\tau$ containing loop with loop head $\ell_L$. A meta trace $\bar{\tau}$ is derived from $\tau$ by replacing the sequence of program statements between the first and last occurrence of $\ell_L$ by a the loop acceleration. The last occurrence of $\ell_L$ is replaced by a new loop exit $\ell_L'$.
\end{mydef}
...
\begin{mydef}
	Given an error trace $\tau$ containing a loop $\tau_L: s_i, s_{i+1}, \ldots, s_j$ with loop head $\ell_L$ that contains $n$ branches which have loop relations $\psi_i$ and loop accelerations $\psi_i^*$ for $i \in 1 \ldots n$. A branching meta trace $\bar{\tau}$ is constructed from $\tau$ by replacing $\tau_L$ with the sequence $\tau_m: \psi_1^*, \varepsilon, \psi_2^*, \varepsilon, \ldots, \psi_n^* $. Where each $\psi_i$ leads to the new loop exit $\ell_L'$ and each $\varepsilon$ from $\ell_L'$ to $\ell_L$.
\end{mydef}
...
\end{figure}
Interpolant sequence $I_{\bar{\tau}_2}: \{ \top,\ x = 0,\ 5 \cdot x \leq 56 \lor x \leq 4,\ 5 \cdot x \leq 56 \lor x \leq 4,\ x \leq 11,\ x = 11,\ \bot\}$ can be used to generate an inductive infeasibility proof for $\tau_2$ by using the post operator as explained before. 

\jw{TODO: in original trace we take conjunction of post(interpolant loop entry I, acceleration I) and post(interpolant loop entry II, acceleration II)}
