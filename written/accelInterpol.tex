...
\begin{equation*}
 	\psi_{L_1}:	\texttt{x <= 100; x := x + 2; y := x + 5} \ = \{(\sigma, \sigma')\ |\ x \leq 100 \land x' = x + 2 \land y = x' + 5 \}
\end{equation*}
...
\jw{todo: x and x' in this does not make sense. Write: $\sigma(x) \leq 100$ and $\sigma'(x) \leq 3223$}

For our example loop relation  	$\psi_{L_1}: \{(\sigma, \sigma')\ |\ x \leq 100 \land x' = x + 2 \land y = x' + 5 \}$ we get the reflexive transitive closure:

\jw{Todo: maybe proof, that every trace is in}

\begin{equation*}
	\psi^*_{L_1}: x = x'\ \land\ y = y'\ \lor\ x \leq 100\ \land\ x' \leq 102\ \land\ x' = x + 2\ \land\ y' \leq y + x'\ \land\ y' = x + 5
\end{equation*}
...
Assume we are given the abstract program $P_3$ with error location $\ell_4$:
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[%
		...
		\node[state, initial left](1){$\ell_1$};
		
		\node[state] (2) [right of=1] {$\ell_2$};
		
		\node[state] (7) [above of=2] {$\ell_3$};
		
		\node[state] (3) [right of=2] {$\ell_4$};
		
		
		\path (1) edge node {$st_0$} (2)
		(7) edge[bend left] node[right] {$st_2$} (2)
		(2) edge[bend left] node[left] {$st_1$} (7)
		(2) edge node {$st_3$} (3)
		;
	\end{tikzpicture}
	\captionof{figure}{Program $P_3$.}
	\label{fig:rect}
\end{figure}
